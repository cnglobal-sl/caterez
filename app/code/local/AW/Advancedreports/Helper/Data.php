...
class AW_Advancedreports_Helper_Data extends AW_Advancedreports_Helper_Abstract
{
    ...
    /**
     * Retrives Default Custom Date Range
     * @return string
     */
...
//    }
    ...
    public function getCustomizedLabels($gridType = null)
    {
        ...
        foreach ($this->getQueue()->getStoredRanges($gridType) as $range) {
            $result[] = array(
                ...
                'label' => $this->__("* From %s to %s (%s)", $this->_prepareHumanDate(@$range['report_from']), $this->_prepareHumanDate(@$range['report_to']), $this->getPeriodLabel(@$range['report_period'])),
            );
        }
        return $result;
    }
    ...
    /**
     * Retrives array with Week Day Nums
     * @return array
     */
    ...
    /**
     * Retrives Last Business Week Label
     * @return string
     */
    ...
    public function getDateFormatWithLongYear()
    {
        if (!method_exists($this->getLocale(), 'getDateFormatWithLongYear')) {
            return preg_replace('/(?<!y)yy(?!y)/', 'yyyy', $this->getLocale()->getTranslation(Mage_Core_Model_Locale::FORMAT_TYPE_SHORT, 'date'));
        } else {
    ...
    /**
     * Retrives digit of week from Zend Date
     *
    ...
    /**
     * Retrives field for date range filter
     *
     * @return array
     */
    ...
    */
    ...
    /**
     * Retrives sorted chain of week day's ISO numbers
     * @return array
     */
