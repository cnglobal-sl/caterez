...
class FME_Fieldsmanager_Model_Fieldsmanager extends Mage_Eav_Model_Entity_Attribute
{
    ...
    public function SaveToFM($mode,$id,$table,$customerId){
	if(!Mage::helper('fieldsmanager')->getStoredDatafor('enable')){
	    return;
	}
	...
	$html='';
		...
		->setOptions($bool);
            ...
            break;    
            case 'date':
		$html .= $this->getjquerydate($NewFieldName,$NewFieldId,$BackEndValue,$class);
...
//                $html .= $date->getHtml();
    ...
    public function getjquerydate($name , $id, $backvalue,$title, $class = null)
	{
		...
		$month = (int)date("m");
		$day = date("d");
		$html = '<input type="text" title="'. $name .'" id="' . $id . '" name="'.$name .'" value="'.$backvalue.'">';
		$html .= '<script>
