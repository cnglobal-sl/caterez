...
class Mage_Customer_AccountController extends Mage_Core_Controller_Front_Action
{
   // gloabal $storecode;
    ...
    protected function _loginPostRedirect()//
    {
        ...
        $session = $this->_getSession();
	//echo '<pre>'; print_r($session->getBeforeAuthUrl()); echo '</pre>'; exit;
        if (!$session->getBeforeAuthUrl() || $session->getBeforeAuthUrl() == Mage::getBaseUrl()) {
            ...
            if (!$session->getAfterAuthUrl()) {
            	$user_email = $session->getCustomer()->getData('email');
            	if(!empty($user_email)){ 
	            ...
	            //	echo '<pre>'; print_r($storecode); echo '</pre>';exit;
	            	$redirectUrl = 'http://'.$_SERVER['SERVER_NAME'].'/?___store=' . $storecode;
	            //	$session->setBeforeAuthUrl(Mage::helper('customer')->getAccountUrl());
	            	$session->setAfterAuthUrl($redirectUrl);
	            	$session2 = Mage::getSingleton('core/session')->getData('_session_validator_data');
	            	Mage::getSingleton('core/session')->setMyValue($storecode);
	            	//echo '<pre>'; print_r($session2); echo '</pre>';exit;
            	}else{
