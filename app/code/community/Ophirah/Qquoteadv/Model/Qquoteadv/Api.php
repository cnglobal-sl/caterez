...
//   );
  ...
  private function _sendProposalEmail($quoteId) {
    ...
    $senderValue = Mage::getStoreConfig('qquoteadv/emails/sender', $_quoteadv->getStoreId());
    if (!empty($senderValue)) {
      $sender['name'] = Mage::getStoreConfig('trans_email/ident_' . $senderValue . '/name', $_quoteadv->getStoreId());
      $sender['email'] = Mage::getStoreConfig('trans_email/ident_' . $senderValue . '/email', $_quoteadv->getStoreId());
