...
jQuery.extend(Datepicker.prototype, {
	...
	_checkExternalClick: function(event) {
		...
		var $target = jQuery(event.target),
			inst = jQuery.datepicker._getInst($target[0]);

		if ( ( ( $target[0].id !== jQuery.datepicker._mainDivId &&
				$target.parents("#" + jQuery.datepicker._mainDivId).length === 0 &&
				!$target.hasClass(jQuery.datepicker.markerClassName) &&
				!$target.closest("." + jQuery.datepicker._triggerClass).length &&
				jQuery.datepicker._datepickerShowing && !(jQuery.datepicker._inDialog && jQuery.blockUI) ) ) ||
			( $target.hasClass(jQuery.datepicker.markerClassName) && jQuery.datepicker._curInst !== inst ) ) {
				jQuery.datepicker._hideDatepicker();
		}
	},
