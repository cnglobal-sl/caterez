...
COGOPC.prototype = {
    ...
    save: function () {
        ...
        if($('isSplit').value == 1){
            ...
            var error_msg = 'The splitting of the following products is not done properly. \n'; var validsplit = true;
            for(y in prod){
                var qty = this.validatecount(y);
                if(prod[y].qty != qty){ validsplit = false;
                    error_msg = error_msg + '"'+ prod[y].name + '" ordered qty is ' + prod[y].qty + ' while splitted total count is ' + qty + ' please adjust ' + (parseInt(prod[y].qty) - parseInt(qty)) + ' \n';
                }
            }
            if(!validsplit){
                isValid = false;
        ...
        $('review-please-wait').show();
         var fieldsmanagerrequest = new Ajax.Request(fieldsmanagerSaveurl,{
                        method:'post',
                        parameters:params
                    });
    ...
    addsplit: function () {
        ...
        $('split-please-wait').show();
         var params = Form.serialize(this.form);
        // params += '&splits=' + $$('#checkout-split-fmefields table').length;
        params += '&splits=' + this.splits;
        var request = new Ajax.Request(spliturl, {

            method: 'post',

            parameters: params,

            onSuccess: this.updateResponse.bind(this),
    ...
    validatecount: function (a) {
        
        var totalqty = 0;
        $$('#checkout-split-fmefields table').each(function(t){
            var tableid = t.id;
            var split = tableid.replace('split','');
            var countid = 'splitcount['+ a + '][' + split + ']';
            if($(countid)){
                var qty = $(countid).value;
    ...
    removesplit: function (a,b) {
        ...
        if(remain){
            $('isSplit').value=1;
            $('addSplitorder').value="SPLIT MORE";
            $('addSplitorder').update("SPLIT MORE");
        }else{
            $('isSplit').value=0;
            $('isSplit').value=="SPLIT ORDER";
            $('addSplitorder').update("SPLIT ORDER");
        }
    },
    updateResponse: function (response) {
        ...
        if (response.html) {
            ...
            $('checkout-split-fmefields').insert( response.html);
            $('isSplit').value=1;
            $('addSplitorder').value="SPLIT MORE";
            $('addSplitorder').update("SPLIT MORE");
            this.splits = response.splits;
